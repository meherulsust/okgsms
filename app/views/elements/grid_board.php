  ...
  <table id="example2" class="table table-bordered table-hover">
    <thead>
      <tr>
        ...
        <th width="10%" align="center"><a href="#">Action</a></th>
        <?php endif?>
      </tr>
    </thead>
    <tbody>
    ...
    <?php foreach ($records as $row): ?>
      <tr class="<?php echo cycle(array('bg1', 'bg')); ?>">
        ...
        <?php if ($field == 'status' or $field == 'custom_set_status' or $field == 'customs_set_status'):
			$status_link = ($field == 'status' ? 'set_status' : ($field == 'custom_set_status' ? 'custom_set_status' : 'customs_set_status'));
			if (strtolower($row[$field]) == 'publish' or strtolower($row[$field]) == 'active' or strtolower($row[$field]) == 'live') {
				$status_title = '<span class="label label-success">' . ucfirst(strtolower($row[$field])) . '</span>';
			} else if (strtolower($row[$field]) == 'unpublish' or strtolower($row[$field]) == 'inactive' or strtolower($row[$field]) == 'draft') {
    ...
    <td width="100" align="center" class='stat_menu stat_menu_id_<?php echo $row['id'] ?>'>
      ...
      <?php elseif ($field == 'send_status'):
			...
			$status_title = '<span class="label label-danger">' . ucfirst(strtolower($row[$field])) . '</span>';
			} else if (ucfirst($row[$field]) == 'Pending') {
			$status_title = '<span class="label label-warning">' . ucfirst(strtolower($row[$field])) . '</span>';
			} else {
			$status_title = ucfirst(strtolower($row[$field]));
		  }?>
        ...
        <td class='actn-btn'>
          <a href="<?=$site_url . $active_controller;?>/edit/<?=encode($row['id'])?>"
            title="Click here for edit this item" class='edit ajax_link'><span class="action_btn btn-primary"><i
                class="fa fa-pencil"></i></span></a>
          <a href="<?=$site_url . $active_controller;?>/del/<?=encode($row['id'])?>"
            title="Click here for delete this item" class='del ajax_link'> <span class="action_btn btn-danger"><i
        ...
        <td class='actn-btn' align="center">
          ...
          <a href="<?=$site_url . $active_controller . '/' . $gactn . '/' . encode($row['id'])?>" class="ajax_link"
            title="Click here for edit this item"><span class="action_btn btn-primary"><i
                class="fa fa-pencil"></i></span></a>
          ...
          <a href="<?=$site_url . $active_controller . '/' . $gactn . '/' . $row['process_id']?>"
            title="Click here for view details"><img src="<?=$image_url?>a_view.gif" width="16" height="13" border="0"
              alt="View" /></a>
          <?php elseif ($gatype === 'del' || $gatype === 'delete'): ?>
          <a href="<?=$site_url . $active_controller . '/' . $gactn . '/' . encode($row['id'])?>"
            title="Click here for delete this item" class='del'><span class="action_btn btn-danger"><i
                class="fa fa-times"></i></span></a>
          <?php else: ?>
          <a href="<?=$site_url . $active_controller . '/' . $gactn . '/' . encode($row['id'])?>"
            title="Click here for <?php echo $gactn ?> this item" class='<?php echo $gactn; ?>'><img
              src="<?=$image_url?>a_<?php echo $gactn; ?>.gif" width="16" height="13" border="0"
              alt="<?php echo $gactn; ?>" /></a>
          <?php endif?>
          <?php endforeach?>
        </td>
        <?php endif?>
      ...
      <tr>
        <td colspan="<?=count($labels) + 2;?>" class="no-record">
          <center>No record is found.</center>
        </td>
      </tr>
      <?php endif?>
    </tbody>
